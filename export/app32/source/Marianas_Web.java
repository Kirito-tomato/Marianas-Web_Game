...
public class Marianas_Web extends PApplet {
...
public void setup() {
  frameRate(30);
  surface.setTitle("Mariana\'s Web");
  surface.setLocation(displayWidth/2 - width / 2, displayHeight/2 - height / 2);
  loadFonts();
  ui.load(loadJSONObject("levels/ui/main_menu.json"));
}
PFont fonts[];
...
public boolean doesFileExist(String path, boolean clone){
...
public void runActionArray(JSONArray actions) {
...
class UI {
  ...
  public void addElement(JSONObject element, boolean updateArray) {
    if (updateArray) {
  ...
  public JSONObject getElementByPosition(float x, float y) {
  ...
  public int getElementIdByPosition(float x, float y) {
    for (int i = 0; i < ElementsData.size(); i++) {
      if (x > ElementsData.getJSONObject(i).getInt("x") && x < ElementsData.getJSONObject(i).getInt("x") + ElementsData.getJSONObject(i).getInt("width") && y > ElementsData.getJSONObject(i).getInt("y") && y < ElementsData.getJSONObject(i).getInt("y") + ElementsData.getJSONObject(i).getInt("height")) {
        return i;
      }
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "Marianas_Web" };
    if (passedArgs != null) {
